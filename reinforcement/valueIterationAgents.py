    ...
    def runValueIteration(self):
        ...
        for s in self.mdp.getStates():
            if not self.mdp.isTerminal(s):
                maxQValue = max([self.getQValue(s, action) for action in self.mdp.getPossibleActions(s)])
                diff = abs(self.getValue(s) - maxQValue)
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(s):
                ...
                for action in self.mdp.getPossibleActions(s):
                    if self.getQValue(s, action) > maxValue:
                        maxValue = self.getQValue(s, action)
                        maxAction = action
                self.values[s] = maxValue
                self.actions[s] = maxAction
                
            for p in predecessors[s]:
                if not self.mdp.isTerminal(p):
                    maxQValue = max([self.getQValue(p, action) for action in self.mdp.getPossibleActions(p)])
                    diff = abs(self.getValue(p) - maxQValue)
                    if diff > self.theta:
