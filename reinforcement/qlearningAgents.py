...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legalActions:
            if self.getQValue(state, action) > bestEstimate:
                bestAction = action
                bestEstimate = self.getQValue(state, action)
        return bestAction
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        qValue = sum(self.weights[feature] * featureVector[feature] for feature in featureVector)
        return qValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        if self.getLegalActions(nextState):
            maxValue = max([self.getQValue(nextState, nextAction) for nextAction in self.getLegalActions(nextState)])
        else:
            maxValue = 0
        featureVector = self.featExtractor.getFeatures(state, action)
