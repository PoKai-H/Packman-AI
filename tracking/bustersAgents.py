...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dis in livingGhostPositionDistributions:
            maxProbPos.append(dis.argMax())
        totalminAction = None
        totalminDis = float('inf')
        for action in legal:
            successorPostion = Actions.getSuccessor(pacmanPosition, action)
            for probPos in maxProbPos:
                if self.distancer.getDistance(successorPostion, probPos) < totalminDis:
                    totalminDis = self.distancer.getDistance(successorPostion, probPos)
                    totalminAction = action
        return totalminAction
