...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # x from 0 to the width of the grid and y from 0 to the height of the grid are legal postions
    legalPos = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    # smallest distance is 0, largest distance is Manhatan distance of X_RANGE + Y_RANGE + MAX_NOISE - 1
    # distance between these two numbers are all legal obeservation distance
    legalObsDis = range(X_RANGE + Y_RANGE + MAX_NOISE - 1)
    variableDomainsDict[PAC] = legalPos
    variableDomainsDict[GHOST0] = legalPos
    variableDomainsDict[GHOST1] = legalPos
    variableDomainsDict[OBS0] = legalObsDis
    variableDomainsDict[OBS1] = legalObsDis
    "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        # Using cumulative_weight to make sure where did the sample land in this distribution
        cumulative_weight = 0.0

        for key, value in self.items():
            cumulative_weight += value
            if cumulative_weight / total >= sample:
                return key

    # In case of floating point inaccuracies, return the last key
        return key
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            # Probability of observing noisyDistance given the actual distance
            probObservation = self.getObservationProb(noisyDistance, pacmanPosition, ghostPosition, jailPosition)
            allPossible[ghostPosition] = probObservation * self.beliefs[ghostPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for pos in self.legalPositions:
            for _ in range(particlesPerPos):
    ...
    def getBeliefDistribution(self):
        ...
        for pos in self.legalPositions:
            ...
            dist[pos] = count / len(self.particles)
        return dist
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            weightedDist[particle] += self.getObservationProb(noisyDis, pacmanPos, particle, jailPos)

        weightedDist.normalize()

        # Check if all particles received zero weight and reinitialize if needed.
