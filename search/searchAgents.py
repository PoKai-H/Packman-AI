        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if next_state in self.corners:
                    if next_state == (self.walls.width-2, 1):
                        new_corners = [True, hold_corners[1], hold_corners[2], hold_corners[3]]
                    elif next_state == (self.walls.width-2, self.walls.height-2):
                        new_corners = [hold_corners[0], True, hold_corners[2], hold_corners[3]]
                    elif next_state == (1, self.walls.height-2):
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    cornerNot = []
    for c in corners:
        if c ==(1,1) and not statecorners[3]:
            cornerNot.append(c)
        if c ==(1,top) and not statecorners[2]:
            cornerNot.append(c)
        if c ==(right, top) and not statecorners[1]:
            cornerNot.append(c)
        if c ==(right, 1) and not statecorners[0]:
            cornerNot.append(c)
